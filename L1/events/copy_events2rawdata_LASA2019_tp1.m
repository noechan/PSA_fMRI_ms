...
for n=1%:numel(panames)
    ...
    if sub<=9
        uulaa_events=strcat('sub-0',num2str(sub),'_ses-001_task-uulaa_acq-multiband_events_check.mat');
        tydyy_events=strcat('sub-0',num2str(sub),'_ses-001_task-tydyy_acq-multiband_events_check.mat');
    else
        uulaa_events=strcat('sub-',num2str(sub),'_ses-001_task-uulaa_acq-multiband_events_check.mat');
        tydyy_events=strcat('sub-',num2str(sub),'_ses-001_task-tydyy_acq-multiband_events_check.mat');
    end
    
    if sub<=9
        sub_path2=fullfile(dest_path, ['sub-0' num2str(sub)],'ses-001','derivatives', 'SPM_prepro','func','events');
    else
        sub_path2=fullfile(dest_path, ['sub-' num2str(sub)],'ses-001','derivatives', 'SPM_prepro','func','events');
    end
    
    sub_path_T=fullfile(source_path, 'Tydyy',panames(n).name, [panames(n).name '_1'], 'func','Triggers');
    cd(sub_path_T)
    ...
    sub_path_U=fullfile(source_path, 'Uulaa',panames(n).name, [panames(n).name '_1'], 'func','Triggers');
    cd(sub_path_U)
